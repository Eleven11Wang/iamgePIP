...
void WB(){
		  ...
		  for(j=5;j<3350;j+=2)
		  {
			...
			//   g_equal=g_hist[pic[i+1][j]];//g histogram equalization
              ...
              Cb_avg+=Cb_equal;
                ...
                // fprintf(foutcr,"%f ",Cr_equal);
		        //fprintf(foutcb,"%f ",Cb_equal);     
          }
      }
      Y_avg=Y_avg/count;
      Cb_avg=Cb_avg/count;
		    ...
		    for(j=5;j<3350;j+=2)
		  {
            ...
            g_equal=pic[i+1][j];
			 //Y_equal=(0.299*r_equal)+(0.587*g_equal)+(0.114*b_equal);
              ...
              (Cr_avg-Cr_equal>0)?(aad_Cr+=Cr_avg-Cr_equal ): (aad_Cr-=Cr_avg-Cr_equal);
			  Cb_equal=(-0.1687*r_equal)-(0.3313*g_equal)+(0.500*b_equal);
              (Cb_avg-Cb_equal>0)?(aad_Cb+=Cb_avg-Cb_equal ): (aad_Cb-=Cb_avg-Cb_equal);
    ...
    printf("select num%d \n",select_num);
    FILE *foutyaccu;
	foutyaccu=fopen("foutyacuu.txt","w+t");
    double Y_accu[16384];
    Y_accu[0]=Y_hist[0];
    for(i=1;i<16384;i++)
	  {
		  Y_accu[i]=Y_hist[i]+Y_accu[i-1];
          fprintf(foutyaccu,"%f ",Y_hist[i]);
    ...
    double r_white,g_white,b_white,e_white;
    double white_num;
    for(i=0;i<2460;i+=2)
	  {
		    for(j=5;j<3350;j+=2)
		  {
             ...
             g_equal=pic[i+1][j];  
              r_equal=pic[i][j+1]; 
			//   r_equal=r_hist[pic[i][j]];
			//   b_equal=b_hist[pic[i+1][j+1]];
			//   g_equal=g_hist[pic[i+1][j]];
            //   e_equal=e_hist[pic[i][j+1]];
